...
const getAllParamsFromUrl = () => {
  ...
  console.info("Extracted URL Parameters:", paramsObject); // Log extracted parameters
  return paramsObject;
};

// Function to get the submetric from the URL based on the path
const getSubmetricFromUrl = () => {
  const currentUrl = window.location.href.toLowerCase();
  const match = currentUrl.match(/\/([^/?]+)(?:\?|$)/); // Adjusted regex to match only the submetric before any query params
  console.info("Extracted submetric from URL:", match ? match[1] : "None found"); // Log the extracted submetric or "None"
  return match ? match[1] : null; // Return the matched submetric or null
};

// Normalize the submetric by converting spaces to hyphens and lowercasing the string
const normalizeSubmetric = (submetric) => {
  return submetric.toLowerCase().replace(/\s+/g, '-');
};
...
const updateSubmetrics = (key, parentElement, metricsData) => {
  ...
  const submetricFromUrl = getSubmetricFromUrl(); // Get the submetric from the URL
  ...
  if (submetricsList && submetrics) {
    ...
    submetrics.forEach(submetric => {
      ...
      if (submetric.url) {
        ...
        // Log the final URL
        console.info(`Final URL with parameters: ${url.toString()}`); // Log final URL with parameters

        // Set the modified URL with parameters
        listItem.setAttribute('data-url', url.toString());
      }

      // Highlight the active submetric if it matches the one from the URL
      if (normalizeSubmetric(submetric.name) === submetricFromUrl) {
        listItem.classList.add('active'); // Add active class to highlight
      }
    ...
    clickableMetrics.forEach(metric => {
      metric.addEventListener('click', (e) => {
        ...
        if (url) {
          ...
          console.info(`Redirecting to updated URL: ${updatedUrl.toString()}`); // Log the redirection URL
...
// Initialize with default metric based on the URL
(async () => {
  const metricsData = await fetchMetricsData();
  if (metricsData) {
    const submetricFromUrl = getSubmetricFromUrl(); // Get the submetric from the URL
    ...
    // Find the main metric that contains the submetric in the JSON data
    for (const [mainMetricKey, submetrics] of Object.entries(metricsData)) {
      // Normalize the submetric name from the JSON to match the URL format
      if (submetrics.some(submetric => normalizeSubmetric(submetric.name) === submetricFromUrl)) {
        defaultMetric = htmlNode.querySelector(`[data-submetrics="${mainMetricKey}"]`);
        console.info(`Found default metric "${mainMetricKey}" matching URL submetric "${submetricFromUrl}".`); // Log the default metric match
        break;
      }
    }

    // If a matching metric is found, set it as the default
    if (defaultMetric) {
      updateSubmetrics(defaultMetric.dataset.submetrics, defaultMetric, metricsData);
      defaultMetric.classList.add('active');
      const toggleSign = defaultMetric.querySelector('.toggle-sign');
      if (toggleSign) {
        toggleSign.textContent = '-';
      }
      const currentSubmetrics = defaultMetric.querySelector('.submetrics');
      if (currentSubmetrics) {
        currentSubmetrics.classList.add('show'); // Show the submetrics of the default metric
      }
    } else {
      console.info("No matching default metric found in the URL."); // Log if no default metric is found
    }
  }
})();
